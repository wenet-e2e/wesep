    ...
    def __call__(self, aud, input_fs, fname="", method="p808"):
        if input_fs != SAMPLING_RATE:
            audio = librosa.resample(
                aud, orig_sr=input_fs, target_sr=SAMPLING_RATE
            )
        else:
            audio = aud
...
class DNSMOS_local:
    ...
    def audio_melspec(
        ...
        if self.convert_to_torch:
            ...
            fb = torch.as_tensor(
                librosa.filters.mel(
                    sr=sr, n_fft=frame_size + 1, n_mels=n_mels
                ).T,
                dtype=audio.dtype,
                device=audio.device,
            )
            mel_spec = torch.matmul(specgram.transpose(-1, -2), fb).transpose(
                -1, -2
            )
            if to_db:
            ...
            if to_db:
                mel_spec = (
                    librosa.power_to_db(mel_spec, ref=np.max) + 40
                ) / 40
        return mel_spec.T
    ...
    def get_polyfit_val(self, sig, bak, ovr, is_personalized_MOS):
        ...
        if is_personalized_MOS:
            p_ovr = poly1d(
                [-0.00533021, 0.005101, 1.18058466, -0.11236046], flag
            )
            p_sig = poly1d(
                [-0.01019296, 0.02751166, 1.19576786, -0.24348726], flag
            )
            p_bak = poly1d(
                [-0.04976499, 0.44276479, -0.1644611, 0.96883132], flag
            )
        else:
